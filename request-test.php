...
$sql = "SELECT *, BOOKREQUEST.notes, reqdate, filetypes, requestid, studentid, title, concat(firstname,\" \",lastname) as name, reqsemester from BOOKREQUEST
	  left join STUDENTS on BOOKREQUEST.studentid = STUDENTS.primarykey WHERE (accepted = 1 or retired='N' or retired='Y') and reqsemester = '" . $semester . "'order BY lastname, firstname, title";
$result= db_query($sql);
