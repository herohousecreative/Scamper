...
$sql = "SELECT *, BOOKREQUEST.notes, reqdate, filetypes, requestid, studentid, title, concat(firstname,\" \",lastname) as name, reqsemester from BOOKREQUEST
	  left join STUDENTS on BOOKREQUEST.studentid = STUDENTS.primarykey WHERE retired='N' order BY reqdate";
$result= db_query($sql);
...
if (db_numrows($result) == 0) {
  ...
  echo "<TD width=20%><strong><font color=\"#FFFFFF\">Request Notes</strong></font></TD>";
  ...
  while ($row = mysql_fetch_array($result)) {
    ...
    echo "<TD><a href=\"pendingrequestdetail.php?requestid=",
	     $row["requestid"],"\">",$row["title"],"</a></TD>";
