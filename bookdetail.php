...
if ($_POST[submit_changes]) {
  ...
  $sql="SELECT primarykey as userkey from USERS where username = '" .$user. "'";
  $result=db_query($sql);
...
if($requestkey) {
	...
	echo "result".$row1["pickedup"]." ".$row1["returned"]. "" .$requestkey;
}
...
<form action="'.$PHP_SELF.'" method="post">
  <table width=100% border="0">
	...
	<tr>
	<td>ISBN (without dashes):<font color="#FF0000">*</font></td>
  ...
  <input type="checkbox" name="DTB" value="ON" '; if (in_array("DTB",$file_formats)) echo "checked"; echo '>DTB
  <input type="checkbox" name="DXB" value="ON" '; if (in_array("DXB",$file_formats)) echo "checked"; echo '>DXB
  <input type="checkbox" name="RTF&plus;" value="ON" '; if (in_array("RTF&plus;",$file_formats)) echo "checked"; echo '>RTF&plus;
...
<br>
  ...
  <table width=100% border="0">
	<tr>
	<td>Learning Ally (RFB&amp;D) Phone: 800.635.1403 Agency #AG77843001</td>
	</tr>
	<tr>
	<td><a href="http://www.learningally.org" target="_blank"> http://www.learningally.org</td>
	</tr>
	<tr>
	<td>Learning Ally (RFB&amp;D) Checked?
          <input type="checkbox" name="rfbd_checked" value="1" '; if ($row["frfbd_checked"]) echo "checked"; echo '>
          </td>
	</tr>
	<tr>
	<td>Learning Ally (RFB&amp;D) Has It?  
          <input type="checkbox" name="rfbd_hasit" value="1" '; if ($row["frfbd_hasit"]) echo "checked"; echo '>
          </td>
	</tr>
	<tr>
	<td>On-hand from Learning Ally (RFB&amp;D)? 
...
<br>
  <table width=100% border="0">
    <tr> 
      <td bgcolor="#666666" colspan="2" font color="#000000">
        <h2><font color="#FFFFFF">Learning Ally (RFB&amp;D) Information</font></h2>
        </td>
    </tr>
    <tr> 
  </table>
...
<table width=100% border="0">
	...
	<tr>
	<td><font color=red>Please edit the pickedup and returned dates using the Edit Request link in</font>
...
{
	$format_array = array("TXT","HTML","PDF","RTF","DOC","DTB","DXB","RTF&plus;","MP3","TEX","VOL","XML");
	...
	$result=edit_book($_POST[FTTEBOOKID],addslashes($_POST[ftitle]),addslashes($_POST[fauthor1ln]),addslashes($_POST[fauthor1fn]),addslashes($_POST[fauthor1mi]),
				...
				addslashes($_POST[bs_promised]),$_POST[bs_hasit],addslashes($_POST[bs_pickedup]),$_POST[scancomplete],$_POST[ocrcomplete],$_POST[checkcomplete],
...
//&requestkey='.$_POST[requestkey].' (This can be added to the Here link above when passing requestkey
/*function request_update()
{
	
	$sql="UPDATE BSRELATION set pickedup='".addslashes($_POST['rfbd_pickedup'])."', returned='".addslashes($_POST['rfbd_returned'])."' where primarykey=".$_POST[requestkey];
	$result = db_query($sql);
	...
	{
		echo '<h2>Learning Ally (RFB&amp;D) Pick up & Returned Date information has been updated successfully</h2>';
	}
